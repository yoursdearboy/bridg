...
it("show page renders correctly", async () => {
  const showPageContextSpy = vi
    .spyOn(Route, "useRouteContext")
    .mockResolvedValue({
      subjectQuery: {
  ...
  const showPageRouteTest = await renderRoute(Route, {
    params: { spaceId: "xxxx", subjectId: "xxxx" },
    context: { subjectQuery: showPageContextSpy.mock },
  });
  expect(showPageRouteTest.asFragment()).toMatchSnapshot();
});
