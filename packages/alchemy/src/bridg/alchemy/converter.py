...
converter = Converter()
terminology: ContextVar[bridg.alchemy.TerminologyService] = ContextVar("terminology")
...
@converter.register_structure_hook
def cd_hook(x, _) -> bridg.alchemy.ConceptDescriptor:
    if isinstance(x, str):
        try:
...
@converter.register_structure_hook
def datavalue_hook(x, cls) -> Optional[bridg.alchemy.DataValue]:
    if x is None:
        return None
    if isinstance(x, dict):
        if "unit" in x:
            return bridg.alchemy.PhysicalQuantity(**x)
        if "code_system" in x:
            return converter.structure(x, bridg.alchemy.ConceptDescriptor)
