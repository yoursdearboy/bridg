...
def test_test(app, server, page: Page):
    ...
    print(data['id'] == result.id)
    print(data['administrative_gender_code']
          == result.administrative_gender_code)
    print(data[
        'death_indicator'] == result.death_indicator)
    print(data['birth_date'] == result.birth_date)
    print(result.administrative_gender_code)
    print(result.death_indicator)
...
def test_person_editname_save(app, server, page: Page):
    url = app.url_for("person.name.edit", person_id=7, id=1)
    request = page.request.get(url)
    expect(request).to_be_ok()
